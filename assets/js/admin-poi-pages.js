...
export function renderPoiRows(pois, imageMap) {
  return pois.map((poi) => {
    ...
    const qrImageUrl = buildPoiQrImageUrl(poi.id);
    const qrScanUrl = buildPoiQrScanUrl(poi.id);
    return `
      <tr>
        <td>${poi.id}</td>
        ...
        <td>
          <a class="image-link" target="_blank" rel="noopener noreferrer" href="${sanitizeText(qrImageUrl)}" title="Mở ảnh QR để quét">
            <img class="poi-thumbnail poi-qr" src="${sanitizeText(qrImageUrl)}" alt="QR ${sanitizeText(poi.name || "POI")}">
          </a>
        </td>
...
export function initDraggablePickerMap(mapId, latInputId, lngInputId, initialLat = null, initialLng = null) {
  const lat = Number(initialLat || DEFAULT_MAP_CENTER[0]);
  const lng = Number(initialLng || DEFAULT_MAP_CENTER[1]);
...
export async function createPoiFromForm() {
  ...
  if (lng < -180 || lng > 180) {
    throw new Error("Longitude phải trong khoảng -180 đến 180");
  }
